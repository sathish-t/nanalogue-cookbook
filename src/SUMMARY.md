# Summary

[Introduction](./introduction.md)

- [CLI usage](./cli.md)
  - [Quick look at your data](./cli/quick_look_at_your_data.md)
  - [Quality control of mod data](./cli/qc_modification_data.md)
  - [Extract raw mod calls](./cli/extract_raw_mod_data.md)
  - [Extracting sequences](./cli/extracting_sequences.md)
  - [Spotting variants in sequence data](./cli/spotting_variants_in_sequence_data.md)
  - [Finding highly modified reads](./cli/finding_highly_modified_reads.md)
  - [Exploring modification gradients](./cli/exploring_modification_gradients.md)
  - [Region-specific analysis](./cli/region_specific_analysis.md)
  - [Recipes](./cli/recipes.md)
  - [CLI Commands Reference](./all_cli_commands.md)
- [Python usage](./python.md)
  - [Python API Reference](./all_python_functions.md)
- [Simulating test data](./simulations/overview.md)
  - [Test data with indels](./simulations/test_data_indels.md)
  - [Test data with random errors](./simulations/test_data_errors.md)
  - [Test data with variants](./simulations/test_data_variants.md)
- [Acknowledgments](./acknowledgments.md)
